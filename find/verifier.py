...
def main(argv):
  ...
  Q = vr.ReadInt(1, 200000)
  vr.ReadEndl()
  
  for i in range(R):
    S = vr.ReadString(string.ascii_uppercase, C, C)
    vr.ReadEndl()
  
  sumLen = 0
  for i in range(Q):
    S = vr.ReadString(string.ascii_uppercase, 1, 200000)
    vr.ReadEndl()
    
    sumLen = sumLen + len(S)
    
  assert sumLen <= 200000, 'The string total length is more than 200,000'
