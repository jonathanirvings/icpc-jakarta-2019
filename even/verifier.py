...
def main(argv):
  ...
  R = vr.ReadIntList(N, 0, 10**6)
  C = vr.ReadIntList(N, 0, 10**6)

  for _ in range(Q):
    ra, ca, rb, cb = map(lambda x: x - 1, vr.ReadIntList(4, 1, N))
    assert (ra, ca) != (rb, cb), 'Found (ra, ca) = (rb, cb)'

    assert (R[ra] + C[ca]) % 2 == 0, 'Cell (ra, ca) contains an odd number'
    assert (R[rb] + C[cb]) % 2 == 0, 'Cell (rb, cb) contains an odd number'
