...
def main(argv):
  ...
  M, Q = vr.ReadIntList(2, 1, 100000)
  
  A = vr.ReadIntList(N, 0, 10**9)
  ...
  assert sumR <= 1000000, "The sum of R must not exceed 200000"
  
  for i in range(Q):
    X = vr.ReadInt(1, M)
    vr.ReadSpace()
    
    Y = vr.ReadInt(1, arrR[X - 1])
    vr.ReadSpace()
  ...
  for i in range(1, len(allArray)):
    assert allArray[i] != allArray[i - 1], "Array A, B and Z must be unique"
